...
class DatetimeField(ft.Row):
    ...
    def _days(self, year: int = dt.date.today().year, month: int = 1) -> None:
        ...
        days = range(1, day + 1)
        self._dropdown(
            "days", days, on_change=self._on_change_wrapper, width=110, hint_text="Day"
        )
    ...
    def _on_change(self, e: ft.TapEvent) -> None:
        ...
        if valid:
            ...
            def dropdown_option(x):
                return ft.dropdown.Option(x)

            self.date_controls_dict["days"].options = list(
                map(dropdown_option, elements)
            )
            self.page.update()
    ...
    def _dropdown(self, name: str, elements: Iterable, **kwargs) -> None:
        ...
        self.date_controls_dict[name] = ft.Dropdown(
            options=list(map(dropdown_option, elements)), **kwargs
        )
    ...
    def value(self, values: Union[list, dict]) -> None:
        ...
        if isinstance(values, list):
            ...
            for key, value in values.items():
                assert (
                    key in self.date_controls_dict[key]
                ), f"{key} not in {self.options}"
